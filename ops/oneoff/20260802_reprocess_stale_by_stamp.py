# =============================================================================
# ONE-OFF — DONE (run 2026-08-02). Do not re-run.
#
# What it did: reprocessed the 1,125 final/ games whose processing_version
# stamp was stale but which reprocess_cfb_json.py never targeted (absent from
# cfb_schedule_master.parquet). Verify pass 2026-08-02 08:07: 0 games stale by
# stamp (logs/cfb_reprocess_logfile_stale-by-stamp.log).
#
# Kept per ops/oneoff/README.md convention; git history is the archive.
# =============================================================================
...
# Lived in python/ when it ran; keep imports resolvable from ops/oneoff/.
...
def main() -> int:
    ...
    ap.add_argument("--force", action="store_true", help="reprocess even if the stamp is current")
    ap.add_argument("--dry-run", action="store_true")
    args = ap.parse_args()

    logger = get_logger("cfb_reprocess", "stale-by-stamp")
    pairs = _stale_pairs(args.force)
    logger.info(f"PROCESSING_VERSION={PROCESSING_VERSION}; {len(pairs)} game(s) stale by stamp")
    if not pairs:
        return 0
